...
function log(message, color = 'white', isError = false) {
    const prefix = chalk.magenta.bold('[ 𝐁𝐋𝐀𝐂𝐊𝐋𝐎𝐑𝐃 𝐈𝐍𝐂 ]');
...
// --- BLACKLORD INC ORIGINAL CODE ---
global.botname = "𝐁𝐋𝐀𝐂𝐊𝐋𝐎𝐑𝐃 𝐈𝐍𝐂"
global.themeemoji = "•"
...
async function startXeonBotInc() {
    ...
    XeonBotInc.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            log('BLACKLORD INC Bot Connected', 'yellow');      
...
function checkEnvStatus() {
    try {
        ...
        if (!fs.existsSync(envPath)) {
            fs.writeFileSync(envPath, '# BLACKLORD INC Bot Environment Variables\nSESSION_ID=\n', 'utf8');
